...
export default function SearchPageClient() {
  ...
  const selectedCategoryList = useMemo(
    () => getMultiParam(searchParams, 'category'),
    [searchParams],
  );
  ...
  const selectedCategories = useMemo(() => new Set(selectedCategoryList), [selectedCategoryList]);
  ...
  const sortedRootOptions = useMemo(() => {
    return categoryRootOptions.slice().sort((a, b) => a.name.localeCompare(b.name));
  }, [categoryRootOptions]);
  ...
  const syncSearchParams = useCallback(
    (nextQuery: string, nextPage: number, manufacturers: string[], categories: string[]) => {
  ...
  }, [query, page, selectedCategoryList, selectedManufacturerList, syncSearchParams]);
  ...
  const renderLabel = (name: string) => {
    ...
    return (
      <>
        {name.slice(0, idx)}
        <span className={styles.categoryHighlight}>{name.slice(idx, idx + term.length)}</span>
  ...
  const renderTree = (nodes: CategoryTreeNode[], depth = 0) =>
    nodes.map((node) => {
      ...
      const includeNode =
        !matchesTree || matchesTree.matches.has(node.path) || matchesTree.ancestors.has(node.path);
      if (!includeNode) {
        return null;
      }
      return (
                ...
                return (
                  ...
                  >
                    ...
                    <span className={styles.categoryOptionText}>
                      ...
                      {showSubtext ? (
                        <span className={styles.categoryOptionSubtext}>{option.topLevelName}</span>
                      ) : null}
                    </span>
                  </label>
                );
